...
def process_logs(log_dir):
    ...
    interval_data = defaultdict(lambda: {
        "target_avg_time": [],
        "response_avg_time": [],
        "url_list": defaultdict(lambda: {
            "count": 0,
            "status_codes": defaultdict(int),
            "unique_clients": set()  # Track unique client IPs
        })
    })
            ...
            with open(os.path.join(log_dir, file), "r") as f:
                for line in f:
                    ...
                    if log_data:
                        ...
                        url_entry["unique_clients"].add(log_data["client_ip"])  # Add client IP to the set
        ...
        final_output[slot] = {
            ...
            "url_list": {
                url: {
                    ...
                    "unique_clients": len(entry["unique_clients"])  # Count of unique client IPs
                }
                for url, entry in data["url_list"].items()
            }
        }
